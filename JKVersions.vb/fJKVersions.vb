...
Public Class fJKVersions
	...
	Private Sub bCancel_Click(ByVal sender As System.Object, ByVal e As System.
	...
	Private Sub Work()
		...
		Dim fwr As HttpWebRequest = HttpWebRequest.Create(
		...
		DownloadFile(fwr, Environment.ExpandEnvironmentVariables(
		...
		ExtractFiles(Environment.ExpandEnvironmentVariables(
						"%TEMP%\JKVersions\Temp\jkupd101.exe"), New String() {"jk.exe"})

		File.Move(Environment.ExpandEnvironmentVariables(
						"%TEMP%\JKVersions\Temp\jk.exe"), Environment.ExpandEnvironmentVariables(
						"%TEMP%\JKVersions\jk.1.01.exe"))

		VerifyHash(Environment.ExpandEnvironmentVariables(
						"%TEMP%\JKVersions\jk.1.01.exe"), My.Resources.JK_1_0_1)

		UpdateProgress(3, "Downloading Jedi Knight 1.01 -> 1.00 patch...")

		Dim hwr As HttpWebRequest = HttpWebRequest.Create(
						"http://www.jkhub.net/project/get.php?id=975")
		hwr.KeepAlive = False
		hwr.Method = WebRequestMethods.Http.Get
		hwr.Timeout = 10000

		Dim fvi As FileVersionInfo = FileVersionInfo.GetVersionInfo(Application.
						ExecutablePath)
		hwr.UserAgent = fvi.ProductName & "/" & fvi.ProductVersion

		DownloadFile(hwr, Environment.ExpandEnvironmentVariables(
		...
		File.Move(Environment.ExpandEnvironmentVariables(
						"%TEMP%\JKVersions\Temp\JK-Extension.dll"), Environment.
		...
		VerifyHash(Environment.ExpandEnvironmentVariables(
						"%TEMP%\JKVersions\jk.Unofficial.Patch.2008.01.16.exe"), My.Resources.
	...
	Private Sub ExtractTools()
		Dim di As New DirectoryInfo(Path.Combine(Path.GetDirectoryName(Application.
		...
		Try
			...
			While read > 0
				...
				If res.ContentLength > -1 Then
					UpdateProgress(pos + (CDbl(fs.Position) / res.ContentLength),
												Nothing)
				End If
			End While
		Catch ex As ThreadAbortException
	...
	Private Sub ExtractFiles(ByVal archive As String, ByVal filenames As String())
		...
		psi.FileName = Path.Combine(Path.GetDirectoryName(Application.ExecutablePath),
						"tools\7za.exe")
	...
	Private Sub PatchFile(ByVal input As String, ByVal output As String, ByVal patch _
		...
		Dim psi As New ProcessStartInfo
		psi.FileName = Environment.ExpandEnvironmentVariables(
						"%TEMP%\JKVersions\Temp\bspatch.exe")
		psi.WorkingDirectory = Environment.ExpandEnvironmentVariables(
						"%TEMP%\JKVersions")
		...
		Try
			File.Delete(Environment.ExpandEnvironmentVariables(
	...
	Private Sub VerifyHash(ByVal filename As String, ByVal hash As Byte())
		...
		For i As Integer = 0 To filehash.Length - 1
			If filehash(i) <> hash(i) Then
				MsgBox("The file was not downloaded or patched correctly.  Aborting.",
	...
	Private Sub UnhandledException(ByVal sender As Object, ByVal e As _
		...
		Dim errorText As String = ex.Source & ": " & ex.GetType.FullName & ": " & ex.
						Message & Environment.NewLine & ex.StackTrace

		While ex.InnerException IsNot Nothing
			ex = ex.InnerException

			errorText = ex.Source & ": " & ex.GetType.FullName & ": " & ex.Message &
		...
		MsgBox("An unexpected condition has been reached in this program and it is " &
						"no longer able to continue running.  Please send the author the " &
						"details from the file that will open so that he can fix it.", MsgBoxStyle.
